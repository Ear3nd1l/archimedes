			...
			$('##question#Attributes.questionInfo.QuestionID#').blur(function() {
				...
				if (fieldValue.length > 0)
				{
					...
					else
					{
						...
						$alertDialog.html('<p class="dialogText" id="pDialogText"><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span><strong>#Attributes.questionInfo.QuestionTitle#</strong> must be a valid phone number.</p>').dialog('open');
					}
				}
			});
		});
